...
ALTER TYPE "public"."booking_details" OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_my_bookings"() RETURNS SETOF "public"."booking_details"
  ...
  SELECT
    ...
    t.ticket_price,
    jsonb_build_object(
      'event_venue_date', ev.event_venue_date,
  ...
  FROM
    public.tickets AS t
  JOIN
    public.events_venues AS ev ON t.events_venues_id = ev.event_venue_id
  JOIN
    public.events AS e ON ev.event_id = e.event_id
  JOIN
    public.venues AS v ON ev.venue_id = v.venue_id
  WHERE
    t.customer_id IN (
